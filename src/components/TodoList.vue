  ...
  <div class="overflow-hidden rounded-lg bg-gray-100 shadow-md dark:bg-gray-700">
    <div class="px-4 py-2">
      ...
      <div v-for="todo in todoList" :key="todo.id" class="flex flex-row w-full border-b py-2 dark:border-gray-500">
        ...
        <div class="flex w-9/12 grid justify-items-start items-center">
          <div v-bind:class = "(todo.completed)?'line-through':''" class="ml-1 text-lg font-base text-secondary">{{ todo.todo }}</div>
        </div>
        <div class="flex w-2/12 justify-items-end items-center">
          <div class="px-1">
            <!-- update icon -->
